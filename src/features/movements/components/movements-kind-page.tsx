...
import { LOTEA_MOVEMENTS_UPDATED_EVENT, notifyMovementsUpdated } from "../movements-events";
import {
  formatMovementDay,
...
const copy: Record<
  MovementKind,
  { title: string; description: string; emptyCard: string; listTitle: string; searchPh: string }
> = {
  income: {
    title: "Ingresos",
    description:
  ...
  expense: {
    title: "Egresos",
    description:
      "Cada egreso es un comprobante E-1, E-2… por proyecto; imprímelo desde Acciones. Fecha contable y lote con tope según saldo. Elige proyecto y mes (opcional), Buscar información, refina por texto y usa +.",
...
export function MovementsKindPage({ kind }: { kind: MovementKind }) {
  ...
  const reloadMovements = useCallback(async () => {
    if (!companyId || !projectId || !loadedProject || loadedProject.id !== projectId) return;
    try {
  ...
  const ofKind = useMemo(
    () => movements.filter((m) => m.kind === kind),
    [movements, kind],
  );
  ...
  const filteredMovements = useMemo(
    ...
    [byMonth, movementsFilter, monthFilter],
  );
  ...
  useEffect(() => {
    setMovementsPage(1);
  }, [monthFilter, movementsFilter, loadedProject?.id, kind]);
  ...
  const someInvoicesMissing = useMemo(
    () =>
      movements.some(
        (m) =>
          !(typeof m.invoiceNumber === "number" && Number.isInteger(m.invoiceNumber) && m.invoiceNumber >= 1),
      ),
    [movements],
  );
  ...
  async function handleSearchProject() {
    ...
    if (!companyId || !projectId) {
      setPanelError("Elige un proyecto.");
      return;
    }
  ...
  async function handleBackfillInvoices() {
    ...
    try {
      ...
      setInvoiceBackfillFeedback({
        variant: "error",
        text:
          err instanceof Error ? err.message : "No se pudo completar la asignación de facturas.",
      });
    } finally {
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
            ...
            {companyId ? (
              <QuickNewMovementButton
                ...
                syncedProjectId={loadedProject && projectId === loadedProject.id ? loadedProject.id : null}
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:gap-5">
                  <div className="min-w-0 flex-1 space-y-2 lg:min-w-[min(100%,20rem)]">
                    <Label htmlFor={`mov-page-project-${kind}`}>Proyecto</Label>
                    <Select
                      value={projectId}
                      onValueChange={(v) => {
                        setProjectId(v);
                  ...
                  <div className="w-full sm:w-auto sm:min-w-[14rem]">
                    <MonthYearFilter
                      ...
                      groupLabel="Mes (opcional). Elige año y mes; debe coincidir con la fecha contable (p. ej. datos 2025 → 2025 + mayo). Vacío = todos los meses."
                    />
                  </div>
                  <Button
                    type="button"
                      ...
                      <MagicCard className="relative overflow-hidden rounded-xl border border-border/80 bg-card/90 p-4 shadow-sm backdrop-blur-sm sm:p-5 lg:p-6">
                        ...
                        <div className="relative z-10 space-y-1">
                          ...
                          <p className="text-sm text-muted-foreground sm:text-[0.9375rem]">
                            Código{" "}
                            <span className="font-mono text-foreground">{loadedProject.code}</span>
                            {" · "}
                            {loadedProject.lotCount} lotes ·{" "}
                            <Badge
                              variant={loadedProject.status === "active" ? "default" : "secondary"}
                            >
                        ...
                        <div className="flex flex-col gap-3 border-b border-border/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-5 sm:py-4">
                          <p className="text-xs font-medium text-muted-foreground sm:text-sm">
                            ...
                            {byMonth.length > 0
                              ...
                              : ""}
                            {monthFilter ? (
                              <span className="text-muted-foreground/90"> (mes filtrado)</span>
                            ) : null}
                          </p>
                            ...
                            <Table className="min-w-[36rem] sm:min-w-0">
                              <TableHeader>
                                <TableRow>
                                  ...
                                  <TableHead className="min-w-20 text-center">Lote</TableHead>
                                  {kind === "income" ? (
                                  ...
                                  <TableRow key={m.id}>
                                    ...
                                    <TableCell className="text-center text-sm">
                                      {m.linkedToLot && m.lotNumber !== null ? (
                                        <span className="tabular-nums font-medium">{m.lotNumber}</span>
                                      ) : (
                                    ...
                                    {kind === "income" ? (
                                      <TableCell className="whitespace-nowrap text-right text-sm tabular-nums text-muted-foreground">
                                        {m.linkedToLot && m.lotValue != null && m.lotValue > 0 ? (
                    ...
                    >
                      <DialogContent className="sm:max-w-md" showCloseButton>
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Esta acción no se puede deshacer. ¿Quieres eliminar el registro{" "}
